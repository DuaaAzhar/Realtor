        ...
        <div className='w-full md:w-[67%] lg:w-[40%] lg:ml-20'>
          <form onSubmit={onSubmit}>
            ...
            <div className='relative mb-6'>
              ...
              {/* PASSWORD ICON */}
              {showPassword ? (
            <AiFillEye  className=' absolute right-3 top-3 text-xl-center cursor-pointer'
            onClick={() => setShowPassword((prevState) => !prevState)}/>
            ):
             <AiFillEyeInvisible className=' absolute right-3 top-3 text-xl-center cursor-pointer'
             onClick={() => setShowPassword((prevState) => !prevState)}/>}
            </div>

            {/* LINK for Sign Up and Forgot password */}
            <div className='flex justify-between whitespace-nowrap text-sm sm:text-lg'> 
              <p className='mb-6'> Already have a account?
                <Link to="/sign-in" className=' text-red-600 hover:text-red-800
                transition ease-in-out ml-1'>Sign In</Link>
              </p>
